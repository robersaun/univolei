...
# Remove handlers que mandam para o console
for h in logging.root.handlers[:]:
    logging.root.removeHandler(h)
# Desliga logging globalmente (inclui streamlit, tornado, etc.)
logging.disable(logging.CRITICAL)
warnings.simplefilter("ignore")                      # tudo
...
# =========================
# IMPORTANTE: GRAVAÇÕES -> Frequência de persistência por destino
# =========================
...
# VARS INICIAIS
# =========================
home_name = away_name = date_str = sets = s = acao = ""
SAVE_TO_GOOGLE = True
QTD_PONTOS_SALVAR_GOOGLE = 5
CARREGAMENTO_INICIAL = True
st.session_state.setdefault("CARREGAMENTO_INICIAL", True)

_logger = logging.getLogger("uv_persist")

def _habilitaPaineis(acao: str, force: bool = False):
    print("@@@ FN habilitaPaineis acao ", acao)

    # Pinta, por padrão, uma única vez por boot
    st.session_state.setdefault("_panels_painted_once", False)

    allowed = {"carregar_jogo", "fechar_set", "finalizar_partida"}
    reason = str(acao or "").strip().lower()
    if reason in allowed:
        force = True

    if st.session_state["_panels_painted_once"] and not force:
        print("@@@ _habilitaPaineis: já pintou neste boot -> return")
        return

    # (debounce leve, opcional)
    st.session_state.setdefault("_last_panels_init_ts", 0.0)
    mid = st.session_state.get("match_id")
    sn  = st.session_state.get("set_number")
    rev = st.session_state.get("data_rev", 0)
    token = f"{reason}|mid={mid}|set={sn}|rev={rev}"
    last_token = st.session_state.get("_last_panels_init_token")
    last_ts    = float(st.session_state.get("_last_panels_init_ts", 0.0))
    now        = _time.perf_counter()
    if not force and last_token == token and (now - last_ts) < 0.6:
        print("@@@ _habilitaPaineis: debounce sem force -> return")
        return
    if force:
        st.session_state["_last_panels_init_token"] = token
        st.session_state["_last_panels_init_ts"]    = now

    # ======== Montagem dos painéis (o seu conteúdo existente) ========
    _titulo_e_CSS()
    barraInicialTime()
    _sets()
    barraInicial()
    ...
    st.session_state["_panels_painted_once"] = True
    ...
    if SAVE_TO_GOOGLE:
        ...
        if should_gs:
            ...
            if statuses and statuses[-1].startswith("GSHEETS:"):
                ...
                print ("ver qtd pontos N ", N)
        else:
            print ("else ver qtd pontos N ", N)
            statuses.append(
...
def _fix_who_scored_from_raw_and_row(raw_line: str, row: dict) -> dict:
    print ("FN _fix_who_scored_from_raw_and_row")
    try:
...
def _normalize_action_in_row(row: dict) -> dict:
    print ("FN _normalize_action_in_row")
...
def _fast_apply_scores_to_row(row: dict):
    print ("FN _fast_apply_scores_to_row")
...
def _persist_fb_on_last_rally(fb_upper: str):
    print ("FN _persist_fb_on_last_rally")
    try:
        fr = st.session_state.frames
...
def resolve_our_roster_numbers(frames: dict) -> list[int]:
    print ("FN resolve_our_roster_numbers")
...
def player_name_by_number(frames: dict, number: int | None) -> str:
    print ("FN player_name_by_number")
...
# =========================
def barraInicialTime():
    print("@@@ FN barraInicialTime")
    top1, top2, top3, top4 = st.columns([2.5, 1, 1, 1])
    with top1:
        if not st.session_state.game_mode:
            st.button("⚙️ Time", key="top_config_team_btn", use_container_width=True, on_click=_cadastroTime,args=(True,),)
    with top2:
        if not st.session_state.game_mode:
            st.button("🆕 Jogo", width='stretch', key="top_new_game_btn", use_container_width=True, on_click=_habilitaCriacaoNovoJogo)
...
def _cadastroTime(show: bool | None = None):
    print("FN@@@ _cadastroTime")

    # permite abrir/fechar via parâmetro
    if show is not None:
        st.session_state["show_config_team"] = show
    ...
    if st.session_state.get("show_config_team", False):
        ...
        with col_close:
            st.button(
                "❌ Fechar",
                key="btn_close_config_team",   # chave única
                use_container_width=True,
                on_click=lambda: st.session_state.update(show_config_team=False)
            )
        ...
        if not our_players.empty:
            ...
            if "display_dataframe" in globals():
                display_dataframe(display_df, height=140)
            else:
                st.dataframe(display_df, height=140, use_container_width=True)
...
# Abertura de partida
# =========================
# REVISAR, ACHO QUE ESSA _list_open_matches NÃO ESTÁ SENDO USADA!      
...
def _reset_boot_flags():
    # Libera um novo ciclo de boot + 1ª pintura
    for k in ["_boot_once", "_boot_defer_panels", "_panels_painted_once"]:
        st.session_state[k] = False
    st.session_state["_panels_initialized"] = False

def _inicialVerificaJogoAbertoOuNovo():
    print("@@@ FN _inicialVerificaJogoAbertoOuNovo st.session_state.match_id = ",
          st.session_state.get("match_id"), " st.session_state.set_number = ",
          st.session_state.get("set_number"), "CARREGAMENTO_INICIAL ", CARREGAMENTO_INICIAL,
          )

    # Guards de boot
    st.session_state.setdefault("_boot_done", False)          # boot finalizado (painéis prontos)
    st.session_state.setdefault("_defer_paint", False)        # botão clicado -> pintar no próximo rerun
    st.session_state.setdefault("_panels_painted_once", False)

    if st.session_state["_boot_done"]:
        print("@@@ _inicialVerificaJogoAbertoOuNovo: já finalizado -> return")
        return

    frames_local = st.session_state.get("frames", {})
    mt_sets = frames_local.get("sets", pd.DataFrame())
    open_list = _list_open_matches(frames_local)

    # Se usuário clicou "Carregar jogo" no seletor: pintar agora e finalizar boot
    if st.session_state["_defer_paint"]:
        st.session_state["_defer_paint"] = False
        st.session_state["_panels_painted_once"] = False  # garante pintura
        print("@@@ _inicialVerificaJogoAbertoOuNovo: defer acionado -> montar painéis")
        _habilitaPaineis("carregar_jogo")
        st.session_state["_boot_done"] = True
        print("@@@ _inicialVerificaJogoAbertoOuNovo: boot concluído.")
        return

    mid = st.session_state.get("match_id")
    sn  = st.session_state.get("set_number")

    # Nenhum match escolhido ainda
    if mid is None:
        if len(open_list) == 0:
            print("NENHUM JOGO PARA CARREGAR! -> abrir criação de novo jogo")
            _habilitaCriacaoNovoJogo()
            st.stop()
            ...
            if sn is None:
                if not mt_sets.empty and (mt_sets["match_id"] == st.session_state.match_id).any():
                    st.session_state.set_number = int(
                        mt_sets.loc[mt_sets["match_id"] == st.session_state.match_id, "set_number"].max()
                    )
                else:
                    st.session_state.set_number = 1

            st.session_state["_panels_painted_once"] = False
            _habilitaPaineis("carregar_jogo 1")
            st.session_state["_boot_done"] = True
            print("@@@ _inicialVerificaJogoAbertoOuNovo: boot concluído.")
            return

        else:
            # Vários jogos: UI seletor (sem chamar _habilitaCarregamentoJogos)
            with st.container():
                st.markdown('<div class="sectionCard">', unsafe_allow_html=True)
                ...
                opts = []
                mt = frames_local.get("amistosos", pd.DataFrame())
                for mid_ in sorted(open_list, reverse=True):
                    row = mt.loc[mt["match_id"] == mid_]
                    if row.empty:
                        continue
                    away_id = int(row.iloc[0]["away_team_id"])
                    away_name = team_name_by_id(frames_local, away_id)
                    label = f"Jogo #{mid_} vs {away_name} — {row.iloc[0]['date']}"
                    opts.append((label, mid_))

                if not opts:
                    st.session_state.match_id = int(open_list[0])
                    st.session_state.set_number = 1
                    st.session_state["_panels_painted_once"] = False
                    _habilitaPaineis("carregar_jogo 2")
                    st.session_state["_boot_done"] = True
                    print("@@@ _inicialVerificaJogoAbertoOuNovo: boot concluído (fallback).")
                    return

                labels = [o[0] for o in opts]
                values = [o[1] for o in opts]
                pick = st.selectbox(
                    "Selecione o jogo para carregar:",
                    options=values,
                ...
                with c1:
                    if st.button("Carregar jogo", use_container_width=True):
                        ...
                        if not mt_sets.empty and (mt_sets["match_id"] == st.session_state.match_id).any():
                            ...
                            st.session_state.set_number = 1
                        # Defer: pinta no próximo rerun
                        st.session_state["_defer_paint"] = True
                        print("@@@ BOTAO: jogo escolhido -> defer painéis no próximo rerun")
                        st.rerun()
    ...
    # Já havia match definido ao abrir
    else:
        print("ELSE ERA PARA CARREGAR ! len(open_list) ", len(open_list))
        if sn is None:
            if not mt_sets.empty and (mt_sets["match_id"] == mid).any():
                st.session_state.set_number = int(
                    mt_sets.loc[mt_sets["match_id"] == mid, "set_number"].max()
                )
            else:
                st.session_state.set_number = 1

        st.session_state["_panels_painted_once"] = False
        _habilitaPaineis("carregar_jogo3")
        st.session_state["_boot_done"] = True
        print("@@@ _inicialVerificaJogoAbertoOuNovo: boot concluído.")
        return

def _habilitaCarregamentoJogos(open_list):
    print("@@@ FN _habilitaCarregamentoJogos ", len(open_list))
    # Evita interferir no boot novo
    if st.session_state.get("_boot_done"):
        print("@@@ _habilitaCarregamentoJogos: boot já finalizado -> return")
        return
    # Opcional: manter prints para rastrear


def _habilitaCriacaoNovoJogo():
    print ("@@@ FN _habilitaCriacaoNovoJogo match_id = ", st.session_state.match_id, " st.session_state.show_config_team ", st.session_state.show_config_team, " st.session_state.show_cadastro = ", st.session_state.show_cadastro)
        ...
        with st.container():
            ...
            with cgjb1:
                st.button("Criar Jogo", key="create_game_btn",
                        on_click=lambda: __criaNovoJogoEHabilitaPaineis(st.session_state.get("new_game_opponent","").strip(), st.session_state.get("new_game_date", date.today())),
                        width='stretch')
            with cgjb2:
...
def _get_or_create_team_id_by_name(frames: dict, name: str) -> int:
    ...
    frames["equipes"] = eq
    return int(next_id)
def __criaNovoJogoEHabilitaPaineis(opp_name: str, dt: date):
    frames_local = st.session_state.frames
    mt = frames_local.get("amistosos", pd.DataFrame())
    if mt.empty:
    ...
    # =========================
    # SEGUE COM DEMAIS CHAMADAS PARA INÍCIO DOS REGISTROS
    # =========================
    _habilitaPaineis("1_JogoCriado")


# =========================
# PLACAR (top) – visível fora do Modo Jogo
# =========================
def _placarModoNormal():
    if not st.session_state.game_mode:
        with st.container():
            st.markdown('<div class="sectionCard">', unsafe_allow_html=True)
            frames = st.session_state.frames
    ...
    if st.session_state.game_mode:
        with st.container():
            ...
            with cR:
                ...
                st.session_state.q_result = st.radio(" ", ["Acerto", "Erro"], horizontal=True,
                    index=["Acerto", "Erro"].index(st.session_state.q_result),
                    key="gm_q_result", label_visibility="collapsed"
                )
            with cP:
                st.markdown("**Posição**")
                st.session_state.q_position = st.radio(" ", ["Frente", "Fundo"], horizontal=True,
            ...
            with cM:
                if not st.session_state.game_mode:
                    ...
                    st.session_state.player_label_mode = st.radio(" ", ["Número", "Nome"], horizontal=True,
...
# Painel principal
# =========================
def _painelPrincipal():
    print("@@@ FN _painelPrincipal()")
    with st.container():
        frames = st.session_state.frames
        df_set = current_set_df(frames, st.session_state.match_id, st.session_state.set_number)
        ...
        with left:
            ...
            with s1:
                st.markdown("**Lado**")
                st.session_state.q_side = st.radio(" ", ["Nós", "Adv"], horizontal=True,
...
def atualizaVarGoogle():
    st.markdown(f"  QTD_PONTOS_SALVAR_GOOGLE: {QTD_PONTOS_SALVAR_GOOGLE}")
...
_inicialVerificaJogoAbertoOuNovo()
_logsFinais()
registroArqLogs()
